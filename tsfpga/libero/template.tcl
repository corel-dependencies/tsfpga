...
set PART     __part__
set FAMILY   __family__
set DEVICE   __device__
set PACKAGE  __package__
set SPEED    __speed__
set TEMP_RANGE  __temp_range__
set TOP      __top__
...
proc fpga_create { PROJECT } {
    global TOOL
    global DESTINATION
    fpga_print "creating the project '$DESTINATION/$PROJECT'"
    switch $TOOL {
        "ise"     {
        ...
        "libero11"  {
            if { [ file exists $DESTINATION/$PROJECT ] } { file delete -force -- $DESTINATION/$PROJECT }
            new_project -name $PROJECT -location $DESTINATION/$PROJECT -hdl {VHDL} -family {ProASIC3L}
            project_settings -vhdl_mode VHDL_2008 -verilog_mode SYSTEM_VERILOG
        }
        "quartus" {
...
proc fpga_open { PROJECT } {
    global TOOL
    global DESTINATION
    fpga_print "opening the project '$DESTINATION/$PROJECT'"
    switch $TOOL {
        ...
        "libero11"  {
            open_project $DESTINATION/$PROJECT/$PROJECT.prjx
        }
        "quartus" {
...
proc fpga_part { PART } {
    global TOOL FAMILY DEVICE PACKAGE SPEED TEMP_RANGE
    ...
    switch $TOOL {
        ...
        "libero11"  {
            set_device -family $FAMILY -die $DEVICE -package $PACKAGE -speed $SPEED -adv_options "TEMPR:$TEMP_RANGE"
        }
        "quartus" {
    ...
    switch $TOOL {
        ...
        "libero11" {
            ...
            if {$ext == "pdc"} {
                ...
                import_component -file "$FILE"
            } else {
                create_links -hdl_source $FILE
                if { "$LIBRARY" != "work" } {
                    add_library -library $LIBRARY
                    add_file_to_library -library $LIBRARY -file $FILE
                }
            }
        }
        "quartus" {
...
proc fpga_include {PATH} {
    ...
    switch $TOOL {
        ...
        "libero" {
            ...
            foreach FILE [glob -nocomplain $PATH/*.svh] {
                create_links -hdl_source $FILE
            }
            build_design_hierarchy
        }
        "libero11" {
            # Verilog Included Files are ALSO added.
            # They must be specified after set_root (see fpga_top)
            foreach FILE [glob -nocomplain $PATH/*.vh $PATH/*/*.vh] {
                create_links -hdl_source $FILE
            }
            foreach FILE [glob -nocomplain $PATH/*.svh $PATH/*/*.svh] {
                create_links -hdl_source $FILE
            }
        }
        "quartus" {
...
proc fpga_top { TOP } {
    ...
    switch $TOOL {
        ...
        "libero11"  {
            ...
            eval $cmd
            # Constraints
            # PDC is only used for COMPILE.
            # SDC could also be used by SYNTHESIZE.  Excluding it because some
            # objects do not exist before synthesis, so unless needed we keep it
            # simple.
            global LIBERO_PLACE_CONSTRAINTS
            global LIBERO_OTHER_CONSTRAINTS
            # if { [info exists LIBERO_OTHER_CONSTRAINTS] } {
            ...
            #     append cmd "-module $TOP -input_type {constraint}"
            #     eval $cmd
            #     set cmd "organize_tool_files -tool {COMPILE} "
            #     append cmd $LIBERO_OTHER_CONSTRAINTS
            #     append cmd "-module $TOP -input_type {constraint}"
            #     eval $cmd
            # }
...
# The first argument, if any, is the destination directory for the project
# directory.
puts $argv
if { $::argc > 0 } {
    global DESTINATION
